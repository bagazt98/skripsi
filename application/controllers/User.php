...
class User extends CI_Controller
{
	...
	public function edit()
	{
		...
		if ($this->form_validation->run() == false) {
			...
			$this->session->set_flashdata('message', '<div class="alert alert-success" role="alert">Profil berhasil di perbarui!!</div>');
			redirect('user');
		}
	}
		...
		if ($this->form_validation->run() == false) {
			...
			if (!password_verify($current_password, $data['user']['password'])) {
				...
				if ($current_password == $new_password) {
					$this->session->set_flashdata('massage', '<div class="alert alert-danger" role="alert">New Password cannot be the same as current password!!</div>');
					redirect('user/changepassword');
				} else {
					...
					$this->db->update('tb_user');
					$this->session->set_flashdata('massage', '<div class="alert alert-success" role="alert">Password Changed!!</div>');
					redirect('user/changepassword');
				}
			}
		}
